...
void UVW_Complete_AdcNotification(void)
{
...
//		Can_17_MCanP_Send();
	...
	Adc3ph_Current_M4_PhyCal();                                                  //相电流分别采样
	...
	Appl_HalStatus_M4_u8 = ((uint8)( Hal_M4_Timestamp_tick & HALSTATUS_MASK ));  //霍尔信号输入应用层

	Testmodel_step();                                                            //应用层状态机启动
	...
	else if ((VePC_S_SubDebugMode_NULL == 0)
		  || (VePC_S_SubDebugMode_NULL == 2)
		  || (VePC_S_SubDebugMode_NULL == 3))
	{
