...
** [cover parentID=DS_AS4XX_PWM001_PI] [/cover]                               **
** [cover parentID=SAS_NAS_ALL_PR746] [/cover]                                **
** [cover parentID=DS_NAS_PWM_PR746] [/cover]                                 **
** [cover parentID=SAS_NAS_ALL_PR128] [/cover]                                **
** [cover parentID=SAS_NAS_ALL_PR748] [/cover]                                **
...
#include "SafetyReport.h"
     ...
     (PWM_USER_MODE_RUNTIME_API_ENABLE == STD_ON) || \
     (PWM_USER_MODE_DEINIT_API_ENABLE == STD_ON))
       #include "Pwm_Protect.h"
#endif
...
#define PWM_E_GROUPID  ((uint8)0x21)

#define PWM_E_PARAM_BUFFER_MARKER  ((uint8)0x22)
...
/* Pwm_17_Gtm_SyncGrpUpdateDuties() Service ID */
#define  PWM_SID_SYNCGRPUPDATEDUTIES ((uint8)0x15)
...
#define PWM_BOTH_EDGES      ((Pwm_17_Gtm_EdgeNotificationType)(3))
  ...
  #define PWM_SFR_INIT_USER_MODE_WRITE32(reg,value)                        \
               (PWM_INIT_USER_MODE_OS_API_WRITE32((reg),(value)))
  /*IFX_MISRA_RULE_19_07_STATUS=To support user modification of OS protected
  calls definition, it is declared as a function like macro*/
  #define PWM_SFR_INIT_USER_MODE_READ32(reg)                               \
               (PWM_INIT_USER_MODE_OS_API_READ32((reg)))
 ...
 initialiser*/
  ...
  #define PWM_SFR_INIT_USER_MODE_MODIFY32(reg,clearmask,setmask)           \
            PWM_INIT_USER_MODE_OS_API_MODIFY32((reg),(clearmask),(setmask));
#else
  /*IFX_MISRA_RULE_19_07_STATUS=To support user modification of OS protected
  calls definition, it is declared as a function like macro*/
  #define PWM_SFR_INIT_USER_MODE_WRITE32(reg,value)                        \
  ...
  /*IFX_MISRA_RULE_19_07_STATUS=To support user modification of OS protected
  calls definition, it is declared as a function like macro*/
  #define PWM_SFR_INIT_USER_MODE_READ32(reg)          (reg)
  /*IFX_MISRA_RULE_19_04_STATUS=First level of indirection is used to support
 modification of OS protected calls, so it cannot be expand to a braced
 initialiser*/
  /*IFX_MISRA_RULE_19_07_STATUS=To support user modification of OS protected
  ...
  { uint32 val ; (val) = (reg); (val) &= (clearmask); (val) |= (setmask);   \
                                                                (reg) = (val);}
#endif /* PWM_RUNNING_IN_USER_0_MODE_ENABLE == STD_ON  */
#else
/*IFX_MISRA_RULE_19_07_STATUS=To support user modification of OS protected
  calls definition, it is declared as a function like macro*/
#define PWM_SFR_INIT_USER_MODE_WRITE32(reg,value)                           \
...
#define PWM_SFR_INIT_USER_MODE_MODIFY32(reg,clearmask,setmask)              \
  ...
  /*IFX_MISRA_RULE_19_07_STATUS=To support user modification of OS protected
  calls definition, it is declared as a function like macro*/
  #define PWM_SFR_RUNTIME_USER_MODE_WRITE32(reg,value)                      \
               (PWM_RUNTIME_USER_MODE_OS_API_WRITE32((reg),(value)))
  /*IFX_MISRA_RULE_19_07_STATUS=To support user modification of OS protected
  calls definition, it is declared as a function like macro*/
  #define PWM_SFR_RUNTIME_USER_MODE_READ32(reg)                             \
               (PWM_RUNTIME_USER_MODE_OS_API_READ32((reg)))
  /*IFX_MISRA_RULE_19_04_STATUS=First level of indirection is used to support
   modification of OS protected calls, so it cannot be expand to a braced
   initialiser*/
  /*IFX_MISRA_RULE_19_07_STATUS=To support user modification of OS protected
    calls definition, it is declared as a function like macro*/
  #define PWM_SFR_RUNTIME_USER_MODE_MODIFY32(reg,clearmask,setmask)           \
            PWM_RUNTIME_USER_MODE_OS_API_MODIFY32((reg),(clearmask),(setmask));
#else
  /*IFX_MISRA_RULE_19_07_STATUS=To support user modification of OS protected
  calls definition, it is declared as a function like macro*/
  #define PWM_SFR_RUNTIME_USER_MODE_WRITE32(reg,value)                      \
  ...
  /*IFX_MISRA_RULE_19_07_STATUS=To support user modification of OS protected
  calls definition, it is declared as a function like macro*/
  #define PWM_SFR_RUNTIME_USER_MODE_READ32(reg)         (reg)
  /*IFX_MISRA_RULE_19_04_STATUS=First level of indirection is used to support
 modification of OS protected calls, so it cannot be expand to a braced
 initialiser*/
  /*IFX_MISRA_RULE_19_07_STATUS=To support user modification of OS protected
  ...
  { uint32 val ; (val) = (reg); (val) &= (clearmask); (val) |= (setmask);    \
                                                                (reg) = (val);}
#endif /* PWM_RUNNING_IN_USER_MODE_ENABLE == STD_ON  */
#else
/*IFX_MISRA_RULE_19_07_STATUS=To support user modification of OS protected
  calls definition, it is declared as a function like macro*/
#define PWM_SFR_RUNTIME_USER_MODE_WRITE32(reg,value)                        \
...
#define PWM_SFR_RUNTIME_USER_MODE_READ32(reg)            (reg)
 ...
 initialiser*/
  ...
  /*IFX_MISRA_RULE_19_07_STATUS=To support user modification of OS protected
  calls definition, it is declared as a function like macro*/
  #define PWM_SFR_DEINIT_USER_MODE_WRITE32(reg,value)                        \
               (PWM_DEINIT_USER_MODE_OS_API_WRITE32((reg),(value)))
  /*IFX_MISRA_RULE_19_04_STATUS=First level of indirection is used to support
 modification of OS protected calls, so it cannot be expand to a braced
 initialiser*/
  /*IFX_MISRA_RULE_19_07_STATUS=To support user modification of OS protected
  ...
  #define PWM_SFR_DEINIT_USER_MODE_MODIFY32(reg,clearmask,setmask)           \
            PWM_DEINIT_USER_MODE_OS_API_MODIFY32((reg),(clearmask),(setmask));
#else
  /*IFX_MISRA_RULE_19_07_STATUS=To support user modification of OS protected
  calls definition, it is declared as a function like macro*/
  #define PWM_SFR_DEINIT_USER_MODE_WRITE32(reg,value)                        \
...
** [cover parentID=DS_MCAL_PWM_0901] [/cover]                                 **
**                                                                            **
*******************************************************************************/
/* PWM027: */
...
typedef uint32 Pwm_17_Gtm_ChannelType; 
...
typedef struct Pwm_17_Gtm_ChannelConfigType
{
  ...
  /* This Pwm_ChannelInfo constitutes
      ...
      PwmChannelInfo =
                        (((ShiftReset) << 22UL)         |   \
                         ((ShiftedSpl) << 21UL)         |   \
                         ((IsReferenceChannel) << 20UL) |   \
                         ((ReferenceChannel) << 13UL)   |   \
                         ((ChannelClass) << 11UL)       |   \
                         ((ChannelCoherency) << 10UL)   |   \
                         ((IdleState) << 9UL)           |   \
                         ((Polarity) << 8UL)            |   \
                         ((AssignedHwUnit))                 \
                        ) 
     Where :
            ...
            other channel
            bit 21:
            Special handling of shifted channel is enabled or not
            bit 20:
            Is the channel reference channel or not
            bit 13-19:
...
typedef struct Pwm_17_Gtm_GrpChanConfigType
{
  /* PWM channel Number*/
  Pwm_17_Gtm_ChannelType PwmChannelNumber;
  /* Tom/Atom Channel Number*/
  uint8 TomAtomNum;
}Pwm_17_Gtm_GrpChanConfigType;

typedef struct Pwm_17_Gtm_GroupConfigType
{
  #if (PWM_SAFETY_ENABLE == STD_ON)
  /* Data pointer Marker to validate Duty Array */
  uint32 PwmDutyBufMarker;
  #endif /* PWM_SAFETY_SUPPORTED */
  /* Group channel configuration*/
  const Pwm_17_Gtm_GrpChanConfigType *GrpChanConfigPtr;
  /* Max channels in the Group */
  uint8 MaxGroupChannels;
  /* Channels in group is of type Tom(0) Or Atom(1) */
  uint8 TomOrAtom ;
  /* ModuleNumber */
  uint8 TomOrAtomModuleNumber;
}Pwm_17_Gtm_GroupConfigType;
...
typedef struct Pwm_17_Gtm_ConfigType
{
  ...
  const Pwm_17_Gtm_GroupConfigType *GroupConfigPtr; 
  /* No of Groups */
  uint8             MaxGroups;
  #endif
...
extern const Pwm_17_Gtm_ConfigType Pwm_ConfigRoot[PWM_17_GTM_CONFIG_COUNT];
...
#endif /* (PWM_DUTY_SHIFT_IN_TICKS == STD_OFF) */
#else
/*IFX_MISRA_RULE_19_07_STATUS=Function like macro used to generate compile time
  error in case of incorrect configuration*/
...
/*IFX_MISRA_RULE_19_07_STATUS=Function like macro used to generate compile time
  error in case of incorrect configuration*/
#define Pwm_17_Gtm_SyncGrpUpdateDuties(PwmGroupId,PwmChDutyPtr)       \
         (ERROR_Pwm_17_Gtm_SyncGrpUpdateDuties_NOT_SELECTED)
...
/*IFX_MISRA_RULE_19_04_STATUS=GetVersionInfo is declared as a function like 
...
**                                                                            **
*******************************************************************************/
extern void Pwm_17_Gtm_Isr(uint8 ChannelNumber);
