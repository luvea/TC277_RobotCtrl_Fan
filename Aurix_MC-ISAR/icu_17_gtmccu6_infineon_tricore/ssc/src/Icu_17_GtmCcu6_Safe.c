...
/*IFX_MISRA_RULE_08_07_STATUS= TempStorageVal variable used
...
/*IFX_MISRA_RULE_19_01_STATUS=Declaration before inclusion of a file. 
  This is from AUTOSAR usage*/
#include "MemMap.h"
...
#if (ICU_INITCHECK_API == STD_ON)
IFX_LOCAL_INLINE uint8 Icu_lChannelInitCheck(void);
#endif

#if (ICU_INITCHECK_API == STD_ON)
#if ((ICU_TIM_SIGMEAS_USED == STD_ON) || (ICU_CCU6_SIGMEAS_USED == STD_ON))
IFX_LOCAL_INLINE uint32 Icu_lSigMesInitCheck
(
...
IFX_LOCAL_INLINE uint32 Icu_lCcu6ChannelInitCheck( 
                       Icu_17_GtmCcu6_ChannelType Channel,
                       const Icu_17_GtmCcu6_ChannelConfigType *LocalIcuChanPtr);
#endif
...
IFX_LOCAL_INLINE uint32 Icu_lCcu6PortInitCheck(
                     const Icu_17_GtmCcu6_ChannelConfigType *LocalIcuChanPtr);  
#endif  
#endif
...
/* [cover parentID=DS_MCAL_ICU_0006] */
#if (ICU_SAFETY_ENABLE == STD_ON)
#if (ICU_INITCHECK_API == STD_ON)
/*******************************************************************************
     ...
     (const Icu_17_GtmCcu6_ConfigType* ConfigPtr)
{
     ...
     (ConfigPtr->Marker == ((uint32)ICU_17_GTMCCU6_MODULE_ID << 16U)))
 {
  ...
  {   
    ...
    if (ErrorFlag == E_OK) 
    { 
      /*Check if SRE bit is not set*/    
      if((ICU_SFR_INIT_USER_MODE_READ32(MODULE_SRC.SCU.SCU.ERU[0].U)& 
                  ICU_SRC_SCU_ERU_SRE_SET_MASK) != ICU_SRC_SCU_ERU_SRE_SET_MASK)
      {
...
#if (ICU_INITCHECK_API == STD_ON)
/******************************************************************************
** Syntax :           IFX_LOCAL_INLINE void Icu_lChannelInitCheck()           **
**                                                                           **
** Service ID:        none                                                   **
**                                                                           **
** Sync/Async:        Synchronous                                            **
...
IFX_LOCAL_INLINE uint8 Icu_lChannelInitCheck(void)
{
  ...
  Icu_17_GtmCcu6_ChannelType CellUsed;
      ...
      ((ICU_QM_TIM_SIGEDGE_USED == STD_ON) && \
      (ICU_EDGE_DETECT_API == STD_ON)) || \
      (ICU_QM_TIM_SIGMEAS_USED == STD_ON) || \
      (ICU_MAX_ASIL_CCU6_CHANNELS != 0U) || (ICU_MAX_QM_CCU6_CHANNELS != 0U))
...
#if ((ICU_TIM_SIGMEAS_USED == STD_ON) || (ICU_CCU6_SIGMEAS_USED == STD_ON)) 
/******************************************************************************
** Syntax :           IFX_LOCAL_INLINE uint32 Icu_lSigMesInit                 **
**                          (                                                **
**                 const Icu_17_GtmCcu6_ChannelConfigType *LocalIcuChanPtr,  **
**                 Icu_17_GtmCcu6_ChannelType Channel,                       **
**                            uint32 DutyIndex                               **
...
**                                                                            **
** Syntax : static Std_ReturnType Icu_lGtmChannelInit(void)                   **
**                                                                            **
** [/cover]                                                                   **
** Service ID:       none                                                     **
**                                                                            **
...
static Std_ReturnType Icu_lGtmChannelInitCheck(void)
{
  ...
  for(Channel = 0U; Channel < MaxChannels; Channel++)
  {
    ...
    if(LocalIcuChanPtr->IcuProperties.AssignedHwUnit == ICU_GTM_OPTION)
    {
     ...
     TempFlag2 |= (uint32)
     ((unsigned_int)ICU_SFR_INIT_USER_MODE_READ32(TimChannelRegPtr->CH_CTRL.U) 
                                                                & (3UL << 24));
     ...
     if (ICU_SFR_INIT_USER_MODE_READ32(TimChannelRegPtr->CH_FLT_RE.U) != 
              (unsigned_int) \
              (LocalIcuChanPtr->IcuProperties.TimChFilterTimeForRisingEdge)) 
     {
        ErrorFlag = E_NOT_OK;
     }
     if (ICU_SFR_INIT_USER_MODE_READ32(TimChannelRegPtr->CH_FLT_FE.U) != 
             (unsigned_int) \
             (LocalIcuChanPtr->IcuProperties.TimChFilterTimeForFallingEdge)) 
     {
       ErrorFlag = E_NOT_OK;
     }  
     if (ICU_SFR_INIT_USER_MODE_READ32(TimChannelRegPtr->CH_IRQ_MODE.U) != 
             (unsigned_int) \
...
#if ((ICU_CCU6_USED == STD_ON) && (ICU_INITCHECK_API == STD_ON))
/*******************************************************************************
** Syntax : IFX_LOCAL_INLINE uint32 Icu_lCcu6ChannelInitCheck(                **
**                  Icu_17_GtmCcu6_ChannelType Channel,                       **
**                  const Icu_17_GtmCcu6_ChannelConfigType *LocalIcuChanPtr)  **
**                                                                            **
** Service ID:       none                                                     **
...
** Return value:     none                                                     **
**                                                                            **
** Description :     Function to check the CC6x registers                     **
**                                                                            **
*******************************************************************************/
IFX_LOCAL_INLINE uint32 Icu_lCcu6ChannelInitCheck(
                        Icu_17_GtmCcu6_ChannelType Channel,
...
#if (ICU_CCU6_USED == STD_ON)
/*******************************************************************************
** Syntax : IFX_LOCAL_INLINE uint32 Icu_lCcu6PortInitCheck(                   **
**                  const Icu_17_GtmCcu6_ChannelConfigType *LocalIcuChanPtr)  **
**                                                                            **
** Service ID:       none                                                     **
**                                                                            **
...
** Return value:     none                                                     **
**                                                                            **
** Description :     Function to check port connections for CCU6x             **
**                                                                            **
*******************************************************************************/
IFX_LOCAL_INLINE uint32 Icu_lCcu6PortInitCheck(
                       const Icu_17_GtmCcu6_ChannelConfigType *LocalIcuChanPtr)
{
  ...
  Temp = ~((uint32)ICU_SFR_INIT_USER_MODE_READ32(Ccu6RegPtr->PISEL0.U) &  
...
**                      E_NOT_OK - if initialization comparison fails         **
**                                                                            **
** Description :     Function to check the CCU6x kernal register              **
**                                                                            **
*******************************************************************************/
static Std_ReturnType Icu_lCcu6T12InitializationCheck(void)
{
      ...
      KernalCount++)
  {
    ...
    {
      TempFlag = (uint32)ICU_SFR_INIT_USER_MODE_READ32(Ccu6RegPtr->CLC.U);
     ...
     if(ICU_SFR_INIT_USER_MODE_READ32(Ccu6RegPtr->T12.U) > 0U)
     {
       ErrorFlag = E_OK;
     }
      /* Verify Enable Interrupt SR0 */
      TempFlag = (uint32)((ICU_SFR_INIT_USER_MODE_READ32(
               MODULE_SRC.CCU6.CCU6[KernalCount].SR0.U) & 0x00000400UL) >> 10U);
